...
from MVT_Project import settings
...
def cart(request):
    param1=request.GET.get("quantity_input1")
    bakeryCards = BakeryCards.objects.filter(id__in=[1,2])
    context = {'bakeryCards': bakeryCards}
    return render(request,"cart.html",context)
def addInCart(request):
    param=[]
    length=len(BakeryCards.objects.all())
    for i in range(length):
        if request.GET.get("quantity_input"+i) is not None:
            param.append(request.GET.get("quantity_input"+i))
    for val in param:
        print(val)
